(function () {
    ...
    function formatNamesLayout(text, options) {
        var cfg = options || {};
        var units = Array.isArray(cfg.units) ? cfg.units.map(normalizeText).filter(Boolean) : [];
        var value = normalizeText(text);
        if (!value && units.length === 0) return '';
        ...
        if (units.length > 0) {
            ...
            for (var u = 1; u < units.length; u++) {
                var connector = (u === units.length - 1) ? 'și' : '&';
                tokens.push(connector + ' ' + units[u]);
            }
        } else {
